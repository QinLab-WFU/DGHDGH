...
from model.clip.simple_tokenizer import SimpleTokenizer as Tokenizer
...
class BaseDataset(Dataset):
    ...
    def get_all_label(self):
        labels = torch.zeros([self.length, len(self.labels[0])], dtype=torch.float32)
...
class PILDataset(BaseDataset):
    """
    SigLIP 专用的数据集类，返回 PIL 图像而不是处理后的张量
    """
    ...
    def __init__(self,
                 ...
                 labels: dict,
                 is_train=True,
                 tokenizer=None,  # SigLIP 会在模型中处理 tokenization
                 maxWords=32,
                 imageResolution=224,
                 npy=False):
        ...
        # SigLIP 只需要基本的 resize，其他处理在模型中完成
        if is_train:
            self.transform = Compose([
                Resize(imageResolution, interpolation=Image.BICUBIC),
                CenterCrop(imageResolution),
    ...
    def _load_image(self, index: int):
        ...
        # 只进行 resize，返回 PIL 图像
        return self.transform(image)

    def _load_text(self, index: int):
        """
        返回原始文本，而不是 tokenized 文本
        SigLIP 会在模型中处理 tokenization
        """
        captions = self.captions[index]
        use_cap = captions[random.randint(0, len(captions) - 1)]
        return use_cap  # 返回原始文本

    def __getitem__(self, index):
        image = self._load_image(index)
        text = self._load_text(index)  # 现在是原始文本
        label = self._load_label(index)
        index = torch.tensor(index, dtype=torch.long)
